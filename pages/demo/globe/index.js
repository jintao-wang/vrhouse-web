  ...
  const initial = () => {
    ...
    function initContinents() {
      continents.current.forEach((item) => {
        create2DFont(item.text, item.position, item.rotate);
      });
    }

    function initCity() {
      city.current.forEach((item) => {
        create2DFont(item.text, item.position, item.rotate, item.color);
        if (item.flag) {
          setTimeout(() => {
    ...
    function create2DFont(text, position, angle, color) {
      if (position.length === 2) {
        const pos = getPosition(position[1] + 90, position[0] - 2, 30.2);
        position = [pos.x, pos.y, pos.z];
      }
      const canvas = makeLabelCanvas(32, text, color);
      ...
      const labelGeometry = new THREE.PlaneBufferGeometry(1.5 * text.length / 1.5, 1.5);
    ...
    const markingPointNew = function (marking) {
      ...
      const geometry = new THREE.ConeBufferGeometry(1, 2, 5);
